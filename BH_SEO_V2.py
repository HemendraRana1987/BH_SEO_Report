...
if not BROTLI_AVAILABLE:
    logger.warning("Brotli library not available. Install with: pip install brotli")
...
class URLCache:
    ...
    def _get_key(self, url: str) -> str:
        ...
        try:
            ...
            normalized_url = urlunparse((parsed.scheme, parsed.netloc.lower(), parsed.path.rstrip('/'), 
...
def fetch_sitemap_urls(sitemap_url: str, site_config: SiteConfig = None) -> tuple:
    """Enhanced sitemap fetching for Asian Paints with Brotli support"""
    ...
    try:
        with get_session() as session:
            ...
            if status_code != 200:
                ...
                if response.content:
                    logger.error(f"Response preview (500 bytes): {response.content[:500]}")
            ...
            # Handle Brotli decompression manually if needed
            ...
            if 'br' in content_encoding:
                if BROTLI_AVAILABLE:
                    try:
                        ...
                        # Fall back to response.text which might handle it
                        content = response.text
                else:
                    logger.warning("Brotli library not available. Using response.text...")
                    content = response.text
            else:
                # Use response.text which handles gzip/deflate automatically
                content = response.text
            ...
            if 'xml' in content_type or content.strip().startswith('<?xml'):
                try:
                    ...
                    logger.warning(f"Content preview: {content[:500]}")
            ...
            if not urls:
                ...
                with open(debug_filename, 'w', encoding='utf-8') as f:
                    ...
                    f.write(f"Content-Encoding: {response.headers.get('content-encoding', 'none')}\n")
                    f.write(f"Content (first 10000 chars):\n{content[:10000]}\n")
                    f.write(f"Raw bytes (first 1000): {raw_content[:1000].hex()}\n")
            ...
            for url in urls:
                try:
                    ...
                    normalized_url = urlunparse((parsed.scheme, parsed.netloc.lower(),
                                               parsed.path.rstrip('/'), parsed.params,
...
def send_site_email_immediately(site: SiteConfig, stats: Dict, sitemap_summary: Dict, 
                               site_execution_time: float, is_first_scan: bool, 
                               cache_stats: Optional[Dict] = None):
    """Send email immediately after site completion without waiting for entire script"""
    print(f"\n📧 SENDING EMAIL FOR {site.name}...")
    logger.info(f"\n{'='*70}")
    logger.info(f"Sending email for {site.name}")
    logger.info(f"{'='*70}")
    ...
    try:
        ...
        logger.error(f"Error sending email for {site.name}: {str(e)}")
        import traceback
        logger.error(traceback.format_exc())
    ...
    with ThreadPoolExecutor(max_workers=SITEMAP_WORKERS) as executor:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                # Add 5-minute break after each sitemap
                if completed < len(sitemaps):
                    print(f"\n⏳ BREAK: Waiting {SITEMAP_BREAK_MINUTES} minutes before next sitemap...")
                    break_seconds = SITEMAP_BREAK_MINUTES * 60
                    
                    for remaining in range(break_seconds, 0, -60):
                        minutes_remaining = remaining // 60
                        print(f"   Next sitemap will start in {minutes_remaining} minutes...")
                ...
                # Still add break even if sitemap failed
                if completed < len(sitemaps):
                    print(f"\n⏳ BREAK: Waiting {SITEMAP_BREAK_MINUTES} minutes before next sitemap...")
                    break_seconds = SITEMAP_BREAK_MINUTES * 60
...
def discover_sitemaps_for_site(site_config: SiteConfig) -> List[str]:
    ...
    if site_config.name == "AsianPaints":
        logger.info(f"Using predefined sitemap list for {site_config.name}")
        return site_config.sitemaps
...
def test_sitemap_connectivity_for_site(site_config: SiteConfig):
    ...
    for sitemap_url in site_config.sitemaps:
        if not sitemap_url:
            continue
        ...
        if results['direct']['status'] == 'Success' or results['with_browser_headers']['status'] == 'Success':
            ...
            if results['with_browser_headers']['error']:
...
def process_site(site: SiteConfig, global_start_time: float) -> tuple:
    ...
    all_sitemaps = [s for s in all_sitemaps if s and s.strip()]
    
    if not all_sitemaps:
        logger.error(f"No valid sitemaps found for {site.name}")
        print(f"❌ No valid sitemaps found for {site.name}")
    ...
    print(f"⏰ Each sitemap will have a {SITEMAP_BREAK_MINUTES}-minute break after completion")
    ...
    return (site, stats, sitemap_summary, None, site_execution_time, is_first_scan, cache_stats)

def process_all_sites(sites: List[SiteConfig], global_start_time: float):
    """Process all sites SEQUENTIALLY with 60-minute breaks and immediate email sending"""
    ...
    for idx, site in enumerate(sites, 1):
        ...
        try:
            ...
            if not working_sitemaps and site.sitemaps:
                ...
                print(f"   Will try to process with enhanced methods...")
            
            result = process_site(site, global_start_time)
            site_obj, stats, sitemap_summary, error_log_path, site_execution_time, is_first_scan, cache_stats = result
            
            send_site_email_immediately(site_obj, stats, sitemap_summary, site_execution_time, is_first_scan, cache_stats)
            ...
            if idx < len(sites):
                next_site = sites[idx]
                print(f"\n{'='*80}")
                print(f"⏳ BREAK: Waiting {SITE_BREAK_MINUTES} minutes before processing {next_site.name}")
                print(f"{'='*80}")
                
                break_seconds = SITE_BREAK_MINUTES * 60
                
                for remaining in range(break_seconds, 0, -60):
                    minutes_remaining = remaining // 60
                    print(f"   Next site will start in {minutes_remaining} minutes...")
                    time.sleep(min(60, remaining))
                
                print(f"✅ Break completed. Starting {next_site.name} now...")
...
def main():
    ...
    try:
        ...
        process_all_sites(SITES, start_time)
        
        print(f"\n{'='*80}")
        print("✅ ALL SITES PROCESSED AND EMAILS SENT!")
        print(f"{'='*80}")
